...
namespace nnet {
            ...
            res_T     res[CONFIG_T::n_row1][CONFIG_T::n_col2]) // n_col1 = n_row2
    {
        ...
        Row: for (int i = 0; i < CONFIG_T::n_row1; i++) {
        
           Col: for (int j = 0; j < CONFIG_T::n_col2; j++) {

               #pragma HLS PIPELINE
               res[i][j] = 0;

                Product: for (int k = 0; k < CONFIG_T::n_col1; k++)
...
res[i][j] = res[i][j] + data1[i][k]*data2[k][j];
            ...
            res_T     res[CONFIG_T::n_row1+CONFIG_T::n_row2][CONFIG_T::n_col2]) // n_col1 must be equal to n_col2
    {
        ...
        for (int i = 0; i < CONFIG_T::n_row1; i++) {
            #pragma HLS PIPELINE
            Col:for (int j = 0; j < CONFIG_T::n_col1; j++) {
//#pragma HLS PIPELINE
        ...
        for (int i = 0; i < CONFIG_T::n_row2; i++) {
            #pragma HLS PIPELINE
            jedi_concat_label6:for (int j = 0; j < CONFIG_T::n_col2; j++) {
//#pragma HLS PIPELINE
    ...
    void jedi_dnn1(
        ...
        for (int cols = 0; cols < CONFIG_T::N_e_p; cols++){
			...
			//for (int cols = 0; cols < 1jedi_dnn1_label2:; cols++) {

            jedi_dnn1_label7:for (int rows = 0; rows < 2 * CONFIG_T::P_p; rows++) {
#pragma HLS PIPELINE
				//#pragma HLS PIPELINE
            ...
            // copy E_col injedi_dnn1_label3:to cols of E
            jedi_dnn1_label8:for(int rows = 0; rows < CONFIG_T::D_e_p; rows++) {
//#pragma HLS PIPELINE
				#pragma HLS UNROLL
    ...
    void jedi_dnn2(
        ...
        for (int cols = 0; cols < CONFIG_T::N_o_p; cols++){
            ...
            // copy O_col injedi_dnn2_label5:to cols of O
