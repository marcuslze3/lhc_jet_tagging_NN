...
int main(int argc, char **argv)
{
  ...
  if (fin.is_open() && fpr.is_open()) {
    while ( std::getline(fin,iline) && std::getline (fpr,pline) ) {
      ...
      nnet::copy_data<float, input_t, 0, N_INPUT_1_1*N_INPUT_2_1>(in, rnn1_input);
      result_t layer9_out[N_LAYER_8];

      //hls-fpga-machine-learning insert top-level-function
      unsigned short size_in1,size_out1;
      myproject(rnn1_input,layer9_out,size_in1,size_out1);
    ...
    input_t rnn1_input[N_INPUT_1_1*N_INPUT_2_1];
    nnet::fill_zero<input_t, N_INPUT_1_1*N_INPUT_2_1>(rnn1_input);
    result_t layer9_out[N_LAYER_8];
