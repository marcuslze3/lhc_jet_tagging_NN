...
def predict(x, Rr, Rr_T, Rs, batch_size):
...
class GraphNet(nn.Module):
    def __init__(self, n_constituents, n_targets, params, hidden, De, Do, 
        ...
        self.fr1 = nn.Linear(2 * self.P + self.Dr, self.hidden).cpu()
        self.fr2 = nn.Linear(self.hidden, int(self.hidden/2)).cpu()
        self.fr3 = nn.Linear(int(self.hidden/2), self.De).cpu()
        self.fo1 = nn.Linear(self.P + self.Dx + self.De, self.hidden).cpu()
        self.fo2 = nn.Linear(self.hidden, int(self.hidden/2)).cpu()
        self.fo3 = nn.Linear(int(self.hidden/2), self.Do).cpu()
        if self.sum_O:
            self.fc1 = nn.Linear(self.Do *1, self.hidden).cpu()
        else:
            self.fc1 = nn.Linear(self.Do * self.N, self.hidden).cpu()
        self.fc2 = nn.Linear(self.hidden, int(self.hidden/2)).cpu()
        self.fc3 = nn.Linear(int(self.hidden/2), self.n_targets).cpu()
...
# 150p model
#mymodel.load_state_dict(torch.load("%s/IN%s_bestmodel5.params" %(loc, '_sumO' if mymodel.sum_O else '')))
...
print("PLOT WEIGHTS")
hls4ml.model.profiling.numerical(model = mymodel)
plt.show()

# QUANTIZE THE MODEL

quantized_model = torch.quantization.quantize_dynamic(
	mymodel.to('cpu'), {torch.nn.Linear}, dtype=torch.half
)

# compare the performance
print("Floating point FP32")
# %timeit mymodel.forward(inputs, hidden)
...
q=print_size_of_model(quantized_model, "fp16")
...
x = 0
"""
for q in quantized_model.state_dict():
	if x==3 or x==7 or x==11 or x==15 or x==19 or x==23 or x==27 or x==31 or x==35 or x==39 :
		#print(quantized_model.state_dict()[q].size())
		# z = quantized_model.state_dict()[q]
		#print(quantized_model.state_dict()[q].size())
		print(quantized_model.state_dict()[q])
	x+=1"""
...
for q in quantized_model.state_dict():
	if x==3 or x==7 or x==11 or x==15 or x==19 or x==23 or x==27 or x==31 or x==35 or x==39 :
		w = quantized_model.state_dict()[q][0]
		b = quantized_model.state_dict()[q][1]
		w = w.data.cpu().numpy().transpose().flatten()
		b = b.data.cpu().numpy().transpose().flatten()
		
		np.savetxt('params_float16/w%s.txt'%(weight_count), [w], delimiter = ',', fmt='%f')
		weight_count = weight_count+1
		
		np.savetxt('params_float16/b%s.txt'%(bias_count), [b], delimiter = ',', fmt='%f')
...
bias_count = 1
weight_count = 1
for i, params in enumerate(quantized_model.parameters()):
	#print('param %s'%(i))
...
with open('params_float16/Rr.h', 'w') as f:
    f.write(np.array2string(Rr, separator=', '))

with open('params_float16/Rr_T.h', 'w') as f:
    f.write(np.array2string(Rr_T, separator=', '))

with open('params_float16/Rs.h', 'w') as f:
...
for batch_idx, mydict in t:
	...
	if count == 0:
		with open('params_float16/input.h', 'w') as f:
			f.write(np.array2string(data.cpu().numpy(), separator=', '))
		
		with open('params_float16/target.h', 'w') as f:
...
if test:
    with torch.no_grad():
        ...
        for batch_idx, mydict in t:
            ...
            out_quant = quantized_model(data.cpu())
            ...
            Rs = mymodel.Rs
            y_hls = predict(data.cpu().numpy(), Rr, Rr_T, Rs, batch_size) # JEDI HLS PREDICTION  
...
print("timing data shape: ",timing_data.shape)
...
mymodel(timing_data)
time_after = float(time.time())
average_time= (time_after-time_before)
