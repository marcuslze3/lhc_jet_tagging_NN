...
os.environ["cpu_VISIBLE_DEVICES"] = "4"
import numpy as np
import h5py
import glob
import itertools
...
class GraphNet(nn.Module):
    def __init__(self, n_constituents, n_targets, params, hidden, De, Do, 
        ...
        self.fr1 = qnn.QuantLinear(2 * self.P + self.Dr, self.hidden, bias=True, weight_quant=WeightQuant,
            bias_quant=BiasQuant, output_quant=ActQuant, return_quant_tensor=True).cpu()
        self.fr2 = qnn.QuantLinear(self.hidden, int(self.hidden/2), bias=True, weight_quant=WeightQuant,
            bias_quant=BiasQuant, output_quant=ActQuant, return_quant_tensor=True).cpu()
        self.fr3 = qnn.QuantLinear(int(self.hidden/2), self.De, bias=True, weight_quant=WeightQuant,
            bias_quant=BiasQuant, output_quant=ActQuant, return_quant_tensor=True).cpu()
        self.fo1 = qnn.QuantLinear(self.P + self.Dx + self.De, self.hidden, bias=True, weight_quant=WeightQuant,
            bias_quant=BiasQuant, output_quant=ActQuant, return_quant_tensor=True).cpu()
        self.fo2 = qnn.QuantLinear(self.hidden, int(self.hidden/2), bias=True, weight_quant=WeightQuant,
            bias_quant=BiasQuant, output_quant=ActQuant, return_quant_tensor=True).cpu()
        self.fo3 = qnn.QuantLinear(int(self.hidden/2), self.Do, bias=True, weight_quant=WeightQuant,
            bias_quant=BiasQuant, output_quant=ActQuant, return_quant_tensor=True).cpu()
        if self.sum_O:
            self.fc1 = qnn.QuantLinear(self.Do *1, self.hidden, bias=True, weight_quant=WeightQuant,
            bias_quant=BiasQuant, output_quant=ActQuant, return_quant_tensor=True).cpu()
        else:
            self.fc1 = nn.Linear(self.Do * self.N, self.hidden).cpu()
        self.fc2 = qnn.QuantLinear(self.hidden, int(self.hidden/2), bias=True, weight_quant=WeightQuant,
            bias_quant=BiasQuant, output_quant=ActQuant, return_quant_tensor=True).cpu()
        self.fc3 = qnn.QuantLinear(int(self.hidden/2), self.n_targets, bias=False, weight_quant=WeightQuant, output_quant=ActQuant).cpu()
    ...
    def assign_matrices(self):
        ...
        for i, (r, s) in enumerate(receiver_sender_list):
            self.Rr[r, i] = 1
            self.Rs[s, i] = 1
        self.Rr = Variable(self.Rr).cpu()
        self.Rs = Variable(self.Rs).cpu()

    def forward(self, x):
        #x = self.quant(x)
        #self.Rr = self.quant(self.Rr)
        #self.Rs = self.quant(self.Rs)
        ...
        #print(B.reshape(-1, 2 * self.P + self.Dr).shape)
        ...
        if self.fr_activation ==2:
            B = self.fr1(B.reshape(-1, 2 * self.P + self.Dr))
            #B = nn.functional.selu(self.fr1(B.reshape(-1, 2 * self.P + self.Dr)))
            B = nn.functional.selu(B)
            B = nn.functional.selu(self.fr2(B))
            E = nn.functional.selu(self.fr3(B).reshape(-1, self.Nr, self.De))            
        elif self.fr_activation ==1:
            B = nn.functional.elu(self.fr1(B.reshape(-1, 2 * self.P + self.Dr)))
            B = nn.functional.elu(self.fr2(B))
            E = nn.functional.elu(self.fr3(B).reshape(-1, self.Nr, self.De))
        else:
            B = nn.functional.relu(self.fr1(B.reshape(-1, 2 * self.P + self.Dr)))
        ...
        if self.fo_activation ==2:
            C = nn.functional.selu(self.fo1(C.reshape(-1, self.P + self.Dx + self.De)))
            C = nn.functional.selu(self.fo2(C))
            O = nn.functional.selu(self.fo3(C).reshape(-1, self.N, self.Do))
        elif self.fo_activation ==1:
            C = nn.functional.elu(self.fo1(C.reshape(-1, self.P + self.Dx + self.De)))
            C = nn.functional.elu(self.fo2(C))
            O = nn.functional.elu(self.fo3(C).reshape(-1, self.N, self.Do))
        else:
            C = nn.functional.relu(self.fo1(C.reshape(-1, self.P + self.Dx + self.De)))
            C = nn.functional.relu(self.fo2(C))
            O = nn.functional.relu(self.fo3(C).reshape(-1, self.N, self.Do))
        del C
        ...
        if self.fc_activation ==2:
            if self.sum_O:
                N = nn.functional.selu(self.fc1(O.reshape(-1, self.Do * 1)))
            else:
                N = nn.functional.selu(self.fc1(O.reshape(-1, self.Do * self.N)))
            N = nn.functional.selu(self.fc2(N))       
        elif self.fc_activation ==1:
            if self.sum_O:
                N = nn.functional.elu(self.fc1(O.reshape(-1, self.Do * 1)))
            else:
                N = nn.functional.elu(self.fc1(O.reshape(-1, self.Do * self.N)))
            N = nn.functional.elu(self.fc2(N))
        else:
            if self.sum_O:
                N = nn.functional.relu(self.fc1(O.reshape(-1, self.Do * 1)))
            else:
                N = nn.functional.relu(self.fc1(O.reshape(-1, self.Do * self.N)))
...
if train:
	...
	# the best model
	torch.save(mymodel.state_dict(), "%s/IN%s_bestmodel_brevitas.params" %(loc, '_sumO' if mymodel.sum_O else ''))
...
mymodel_quantized = torch.quantization.quantize_dynamic(
    mymodel, {nn.LSTM, nn.Linear}, dtype=torch.qint8
)
...
# compare the sizes
f=print_size_of_model(mymodel,"fp32")
q=print_size_of_model(mymodel_quantized,"int8")
...
time_model_evaluation(mymodel_quantized,	

print("EXAMPLE QUANTIZED!") """
"""
# STATIC QUANTIZATION

#deploymentmymodel.qconfig = torch
mymodel = mymodel.to('cpu')
mymodel.eval()
#mymodel.fuse_model()
mymodel.qconfig = torch.quantization.get_default_qconfig
mymodel_prepared = torch.quantization.prepare(mymodel)
mymodel_quantized = torch.quantization.convert(mymodel_prepared)
"""
"""
# load onto CPU
device = torch.device('cpu')
model_float = mymodel = GraphNet(nParticles, len(labels), params, int(x[0]), int(x[1]), int(x[2]), 
                   fr_activation=int(x[3]),  fo_activation=int(x[4]), fc_activation=int(x[5]), optimizer=int(x[6]), verbose=True)
                
model_float.load_state_dict(torch.load("%s/IN%s_bestmodel5.params" %(loc, '_sumO' if mymodel.sum_O else ''), map_location=device))
mymodel_quantized = torch.quantization.quantize_dynamic(model_float, {torch.nn.Linear}, dtype=torch.qint8)
...
print("unquantized size: ")
#print_model_size(mymodel_quantized)
...
if test:
    with torch.no_grad():
        ...
        for batch_idx, mydict in t:
            ...
            #data.to('cpu')
            #mymodel_quantized.to('cpu')
            #out_quantized = mymodel_quantized(data)
            out_test = mymodel(data)
            print(torch.max((torch.flatten(data))))
            #print(data.shape)
            #print(data)
            Rr = mymodel.Rr
            Rr_T = torch.transpose(mymodel.Rr, 0, 1).contiguous()
            Rs = mymodel.Rs
            #y_hls = predict(data.numpy(), Rr, Rr_T, Rs) # JEDI HLS PREDICTION   
            #print(y_hls)
            #print(out_test)
            ...
            out_tests  += [out_test]
            targets  += [target]
            #out_quantizeds += [out_quantized]
            
            l_test = loss(out_test, target)
            acc_test = accuracy(out_test, target)
            #acc_quantized = accuracy(out_quantized, target)
...
if roc == True:
	...
	for k in range(kfold):
		...
		for j in torch.split(X_test, batch_size):
			a = models[k](j.cpu()).cpu().data.numpy()
			lst.append(a)
	...
	with open('%s_brevitas_quantized/IN_%i_ROC_withSumO_fpr.pickle' %(sys.argv[1], nParticles), 'wb') as handle:
		pickle.dump(fpr, handle, protocol=pickle.HIGHEST_PROTOCOL)
	with open('%s_brevitas_quantized/IN_%i_ROC_withSumO_tpr.pickle' %(sys.argv[1], nParticles), 'wb') as handle:
		pickle.dump(tpr, handle, protocol=pickle.HIGHEST_PROTOCOL)
	with open('%s_brevitas_quantized/IN_%i_ROC_withSumO_AUC.pickle' %(sys.argv[1], nParticles), 'wb') as handle:
