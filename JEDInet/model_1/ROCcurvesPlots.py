...
for model in models_nokfold:
    print("\n") 
    with open('50_brevitas_quantized/IN_50_ROC_withSumO_AUC.pickle', 'rb') as handle:
        AUC = pickle.load(handle)
    for label in labels:
        myAUC = []
...
if plot:
	...
	for i in range(len(labels)):
		...
		for i in range(len(models_nokfold)):
			...
			with open("50_brevitas_quantized/%s_ROC_fpr.pickle" %model, 'rb') as handle:
				fpr = pickle.load(handle)
			with open("50_brevitas_quantized/%s_ROC_tpr.pickle" %model, 'rb') as handle:
				tpr = pickle.load(handle)
