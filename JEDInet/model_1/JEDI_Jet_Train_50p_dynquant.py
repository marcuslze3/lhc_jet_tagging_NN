...
if train:
	...
	# the best model
	torch.save(mymodel.state_dict(), "%s/IN%s_bestmodel_50p_dynquant.params" %(loc, '_sumO' if mymodel.sum_O else ''))
...
mymodel.load_state_dict(torch.load("%s/IN%s_bestmodel_50p_dynquant.params" %(loc, '_sumO' if mymodel.sum_O else '')))
...
if test:
    with torch.no_grad():
        ...
        for batch_idx, mydict in t:
            ...
            #t.set_description("test batch loss = %.5f, acc = %.5f, quant acc = %.5f" % (loss_test_item, acc_test, acc_quant))
