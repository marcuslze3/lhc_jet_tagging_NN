...
mymodel.load_state_dict(torch.load("%s/IN%s_bestmodel7.params" %(loc, '_sumO')))
models.append(mymodel)
...
if test:
	...
	X_test = np.array([])
	Y_test = np.array([])
	#glob.glob("/mnt/ccnas2/bdp/mzl20/proj_conda/lhc_jet_tagging_NN/JEDInet/model_1/150p_dataset/train/jetImage*_%sp*.h5" %nParticles)
	inputFiles = glob.glob("/mnt/ccnas2/bdp/mzl20/proj_conda/lhc_jet_tagging_NN/JEDInet/model_1/150p_dataset/val/jetImage*_%sp*.h5" %nParticles)   
	...
	acc = accuracy_score(true_test, np.argmax(predict_test, axis=1))
