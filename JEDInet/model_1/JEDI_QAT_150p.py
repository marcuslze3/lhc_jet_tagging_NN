...
# defines two floating point formats

bit_8 = FloatingPoint(exp=5, man=2)
bit_16 = FloatingPoint(exp=6, man=9)
"""
bit_23 = FloatingPoint(exp=6, man=16)
bit_34 = FloatingPoint(exp=8, man=23)"""
"""
# defines two fixed point formats
bit_16 = FixedPoint(wl=16, fl=10)
bit_32 = FixedPoint(wl=32, fl=20)
"""

#define quantization functions
weight_quant = quantizer(forward_number=bit_16, forward_rounding="nearest")
grad_quant = quantizer(forward_number=bit_16, forward_rounding="nearest")
momentum_quant = quantizer(forward_number=bit_16, forward_rounding="stochastic")
acc_quant = quantizer(forward_number=bit_16, forward_rounding="stochastic")

# define a lambad so that the Quantizer module can be duplicated easily
act_error_quant = lambda: Quantizer(forward_number=bit_16, backward_number=bit_16, forward_rounding="nearest", backward_rounding="nearest")
...
class GraphNet(nn.Module):
    def __init__(self, quant, n_constituents, n_targets, params, hidden, De, Do, 
    ...
    def forward(self, x):
        ...
        if self.fr_activation ==2:
            ...
            B = self.quant(B)
            B = nn.functional.selu(self.quant(self.fr2(B)))
            B = self.quant(B)
...
if True:
	...
	inputFiles = glob.glob("/mnt/ccnas2/bdp/mzl20/proj_conda/lhc_jet_tagging_NN/JEDInet/model_1/150p_dataset/train/jetImage*_%sp*.h5" %nParticles)
...
mymodel = GraphNet(act_error_quant, nParticles, len(labels), params, int(x[0]), int(x[1]), int(x[2]), 
                   fr_activation=int(x[3]),  fo_activation=int(x[4]), fc_activation=int(x[5]), optimizer=int(x[6]), verbose=True)

# USE LOW-PRECISION OPTIMIZER
optimizer = SGD(mymodel.parameters(), lr=0.0001, momentum=0.9, weight_decay=5e-4)
optimizer_q = OptimLP(optimizer, weight_quant=weight_quant, grad_quant=grad_quant, momentum_quant=momentum_quant, acc_quant=acc_quant, grad_scaling=1/1000)
...
if train:
	...
	# the best model
	torch.save(mymodel.state_dict(), "%s/IN%s_bestmodel7.params" %(loc, '_sumO' if mymodel.sum_O else ''))
...
mymodel.load_state_dict(torch.load("%s/IN%s_bestmodel7.params" %(loc, '_sumO' if mymodel.sum_O else '')))

# MAKE SURE DATASET PARTICLE SIZE MATCHES TEST SET PARTICLE SIZE
inputTestFiles = glob.glob("/mnt/ccnas2/bdp/mzl20/proj_conda/lhc_jet_tagging_NN/JEDInet/model_1/150p_dataset/val/jetImage*_%sp*.h5" %nParticles)
