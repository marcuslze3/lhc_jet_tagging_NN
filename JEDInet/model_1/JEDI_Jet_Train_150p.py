...
if train:
	...
	# the best model
	torch.save(mymodel.state_dict(), "%s/IN%s_bestmodel5.params" %(loc, '_sumO' if mymodel.sum_O else ''))
...
if test:
    with torch.no_grad():
        ...
        for batch_idx, mydict in t:
            ...
            t.set_description("test batch loss = %.5f, acc = %.5f, hls acc = %.5f" % 
				(loss_test_item, acc_test, acc_hls))
        ...
        #out_hls = torch.cat(torch.from_numpy(out_hls),0)
        ...
        out_predict = np.argmax(out_tests, 1)
        out_predict_encoded = le.fit_transform(out_predict)
        out_tests_encoded = to_categorical(out_predict_encoded, 5)
        
        y_true = to_categorical(target_encoded, 5)
        print(targets.shape)
        print(targets)
        print(y_true)
        print("====================================================")
        print(out_predict.shape)
        print(out_tests)
        print(out_predict)
        print(out_tests_encoded)
        
        # write results
        np.savetxt('y_true.txt', y_true)
        np.savetxt('y_pred_selu.txt', out_tests)
        np.savetxt('y_pred.txt', out_tests_encoded)
		"""
