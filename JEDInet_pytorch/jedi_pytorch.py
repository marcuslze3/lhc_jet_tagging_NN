# CODE FROM:
# https://github.com/jmduarte/JEDInet-code/blob/master/python/JetImageClassifier_IN_FinalTraining.py
...
class GraphNet(nn.Module):
    def __init__(self, n_constituents, n_targets, params, hidden, De, Do,
                 fr_activation=0, fo_activation=0, fc_activation=0, optimizer=0,
                 verbose=False):
    ...
    def assign_matrices(self):
        ...
        receiver_sender_list = [i for i in
    ...
    def forward(self, x):
        ...
        if self.fo_activation == 2:
            C = nn.functional.selu(
                self.fo1(C.view(-1, self.P + self.Dx + self.De)))
    ...
    def tmul(self, x, y):  # Takes (I * J * K)(K * L) -> I * J * L
        x_shape = x.size()
        y_shape = y.size()
        return torch.mm(x.view(-1, x_shape[2]), y).view(-1, x_shape[1],
                                                        y_shape[1])
...
def stats(predict, target):
    ...
    for i in vals:
        ...
        i, correct, len(pv), correct * 100.0 / len(pv)))
    print("Overall: %s/%s = %s%%" % (
    sum(p_vals == t), len(t), sum(p_vals == t) * 100.0 / len(t)))
...
if os.path.isdir('/data/ML/mpierini'):
    inputTrainFiles = glob.glob(
        "/data/ML/mpierini/hls-fml/jetImage*_%sp*.h5" % nParticles)
    inputValFiles = glob.glob(
        "/data/ML/mpierini/hls-fml/VALIDATION/jetImage*_%sp*.h5" % nParticles)
    ...
    inputTrainFiles = glob.glob(
        "/imdata/NEWDATA/jetImage*_%sp*.h5" % nParticles)
    inputValFiles = glob.glob(
        "/imdata/NEWDATA/VALIDATION/jetImage*_%sp*.h5" % nParticles)
elif os.path.isdir('/home/jduarte'):
    inputTrainFiles = glob.glob(
        "/home/jduarte/NEWDATA/jetImage*_%sp*.h5" % nParticles)
    inputValFiles = glob.glob(
    ...
    inputValFiles = glob.glob(
        "/data/shared/hls-fml/NEWDATA/VALIDATION/jetImage*_%sp*.h5" % nParticles)
...
mymodel = GraphNet(nParticles, len(labels), params, int(x[0]), int(x[1]),
                   int(x[2]),
                   fr_activation=int(x[3]), fo_activation=int(x[4]),
                   fc_activation=int(x[5]), optimizer=int(x[6]), verbose=True)
...
for i in range(n_epochs):
    ...
    if all(loss_val[max(0, i - patience):i] > min(
            np.append(loss_val[0:max(0, i - patience)], 200))) and i > patience:
...
torch.save(mymodel.state_dict(),
           "%s/IN%s_bestmodel.params" % (loc, '_sumO' if mymodel.sum_O else ''))
© 2021
GitHub, Inc.
Terms
Privacy
